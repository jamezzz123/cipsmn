...
const ExemptionPayment = () => {
  ...
  const exemptionAmountChangeHandler = (e) => {
    console.log(e.target.value.trim())
    setExemptionAmount(e.target.value.trim())
  }

  const fetchExemption = useCallback(() => {
    setIsLoading(true)
    axios
    .get('/user/payment-settings/get/4', {
        headers: {
      ...
      .then((response) => {
        ...
        if (response.status === 201 || response.status === 200) {
          ...
          setExemptionAmount(response.data.payment_settings.amount)
        } else {
  ...
  const onSubmitHandler = async (e) => {
    ...
    setIsLoading(true)
    axios
    .put(`admin/payment-settings/edit/4`, data, {
      headers: {
                ...
                <div className='form-group row pb-4'>
                  ...
                  <div className='col-lg-6'>
                    <input
                      ...
                      onChange={exemptionAmountChangeHandler}
                    />
                  </div>
                </div>
              </div>
...
export default ExemptionPayment
